...
if __name__ == '__main__':
    ...
    hospital_names = ["D", "F", "G", "N", "All"]
    criterion = nn.CrossEntropyLoss().to(device)
    result_dict = {}
    for hospital_name in hospital_names:
        print(f'Test on hospital dataset: {hospital_name}')
        txt_path = txt_name_prefix + hospital_name + '.txt'
        ann_txt_dir_val = os.path.join(root_dir, txt_path)
    
        validate_dataset = MyDataset(root_dir = root_dir, ann_txt_dir = ann_txt_dir_val, transform=data_transform["val"])
        val_num = len(validate_dataset)
        validate_loader = torch.utils.data.DataLoader(validate_dataset,batch_size=32, shuffle=False,num_workers=8)
            
        best_acc_path = "/mnt/raid/tangzichen/Liver/full_results_D_0.001/best_resNet50.pth"
        ...
        print(f'On Hospital dataset {hospital_name} acc: {acc}')
